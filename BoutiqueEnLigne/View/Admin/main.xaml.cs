...
namespace BoutiqueEnLigne.View.Admin
{
    ...
    public partial class Main : TabbedPage
    {
        ...
        public async void LoadCommandes()
        {
            var Commandes = await dataBase.ObtenirCommandes();
            commandListView.ItemsSource = Commandes;
        }
        ...
        public async void details(object sender, EventArgs e)
        {
            var item = sender as Button;
            var Commande = item.CommandParameter as Commande;
            await Navigation.PushModalAsync(new Details(Commande));
            LoadCategories();
        }
    }
}